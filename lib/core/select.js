...
class Select extends SqlObject {
    ...
    groupBy(fields) {
        ...
        for (let i = 0; i < arguments.length; i++)
            this._groupby.push(arguments[i].isColumn || arguments[i].isRaw ? arguments[i] :
    ...
    orderBy(fields) {
        if (!fields) return this;
        for (let i = 0; i < arguments.length; i++)
            this._orderby.push(arguments[i].isOrder || arguments[i].isRaw ? arguments[i] :
                new Order(String(arguments[i])));
        return this;
    }
