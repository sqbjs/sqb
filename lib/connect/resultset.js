...
class ResultSet extends EventEmitter {
   ...
   * @param {string|undefined} [options.naming]
   * @param {Function} callback
   * @private
   */
            ...
            for (let row of rows) {
              if (--skipRows < 0) {
                out.push(options.objectRows ?
                    self._arrayRowToObj(row, options.ignoreNulls, options.naming) :
                    row.slice());
              }
            }
            if (!needClose)
   ...
   * @private
   */
  _arrayRowToObj(row, ignoreNulls, naming) {
    const out = {};
    const self = this;
