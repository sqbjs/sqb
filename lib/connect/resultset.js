...
class ResultSet extends EventEmitter {
  ...
  constructor(connection, options) {
    ...
    self._fieldNaming = options.fieldNaming;
  ...
  getRows(rowStart, numRows, options, callback) {
    ...
    if (options.ignoreNulls === undefined)
      options.ignoreNulls = self._ignoreNulls;
    self._taskQueue.enqueue((nextTask) => {
  ...
  _move(options, callback) {
    const self = this;
    const arrayResult = options.numRows;
    const nRows = options.numRows || 1;
   ...
   * @param {string|undefined} [options.fieldNaming]
   * @param {Function} callback
   * @private
   */
  _fetchRows(rowStart, numRows, options, callback) {
    ...
    if (options.objectRows === undefined)
      options.objectRows = self._objectRows;
    if (options.fieldNaming === undefined)
      options.fieldNaming = self._fieldNaming;
            ...
            for (let row of rows) {
              if (--skipRows < 0) {
                out.push(options.objectRows ?
                    self._arrayRowToObj(row, options.ignoreNulls, options.fieldNaming) :
                    row.slice());
              }
            }
            if (!needClose)
   ...
   * @param {String} fieldNaming
   * @return {Object}
   * @private
   */
  _arrayRowToObj(row, ignoreNulls, fieldNaming) {
    const out = {};
    const self = this;
    ...
    metaData.fields.forEach((field, idx) => {
      ...
      if (!(v === null && ignoreNulls)) {
        const name = fieldNaming === 'lowercase' ?
            field.name.toLowerCase() :
            (fieldNaming ===
            'uppercase' ? field.name.toUpperCase() : field.name);
        out[name] = v;
      }
    });
    return out;
