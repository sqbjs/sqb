...
function Pool(config) {
  ...
  const driver = extensions.createDriver(config);
  if (!driver)
    throw new ArgumentError('No connection adapter registered for dialect `%s`', config.dialect);
  /* istanbul ignore else */
  if (driver.paramType !== undefined)
    config.paramType = driver.paramType;
  defineConst(this, {
    _dialect: config.dialect,
    _user: config.user,
    _schema: config.schema,
    _driver: driver,
    _npool: _makePool(this, config && config.pool)
  }, false);
  defineConst(this, {
    _metaData: null
  }, {
...
Pool.prototype = {
  ...
  get metaData() {
    if (!this._metaData)
      this._metaData = new DBMeta(this, this.dialect);
