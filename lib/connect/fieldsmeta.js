...
class FieldsMeta {
  ...
  constructor(options) {
    this.fields = [];
    this._naming = options && options.naming;
    this.objectRows = options && options.objectRows;
  }
  ...
  add(obj) {
    ...
    if (obj.name && this.naming) {
      obj.name = this.naming === 'lowercase' ? obj.name.toLowerCase() :
          (this.naming === 'uppercase' ? obj.name.toUpperCase() :
              obj.name);
    }
  ...
  asObject(options) {
    ...
    for (let i = 0; i < this.fields.length; i++) {
      ...
      if (options && options.naming &&
          options.naming !== this.naming)
        name = options.naming === 'lowercase' ? name.toLowerCase() :
            (options.naming === 'uppercase' ? name.toUpperCase() : name);
  ...
  toJSON() {
    return this.objectRows ? this.asObject() : this.fields;
  }
