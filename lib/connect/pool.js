...
class DbPool extends EventEmitter {
  ...
  constructor(config) {
    ...
    // Create serializer plugin
    for (const plugin of plugins.items) {

      if (typeof plugin.createSerializer === 'function') {
        self.serializer = plugin.createSerializer(config);
        if (self.serializer) break;
      }
    }
    assert(self.serializer, `No serializer plugin registered for dialect "${config.dialect}"`);

    // Create nasted pool plugin
    for (const plugin of plugins.items) {
      if (typeof plugin.createPool === 'function') {
        self.nastedPool = plugin.createPool(config);
        self._plugin = plugin;
        if (self.nastedPool) break;
      }
    }
