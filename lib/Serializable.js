...
Serializable.serializeFallback = function(ctx, type, o, defFn) {
  if (ctx.serializeHooks) {
    var hook;
    var x;
    for (var i = 0; i < ctx.serializeHooks.length; i++) {
      hook = ctx.serializeHooks[i];
      x = hook(ctx, type, o, defFn);
      if (x)
        return x;
    }
  }
  if (ctx.extension && ctx.extension.serialize)
    return ctx.extension.serialize(ctx, type, o, defFn);
  return defFn();
};
