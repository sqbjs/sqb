...
class Serializer {
  ...
  get prmGen() {
    return this.parent ? this.parent.prmGen : ('generated_parameter_' + this._prmGen++);
  }
  ...
  build(statement, values) {
    ...
    return {
      sql: flattenText(sql, {singleLine: !this.prettyPrint}),
  ...
  _serializeCase(obj, inf) {
    if (obj._expressions.length) {
      ...
      return flattenText(out, {singleLine: out.length < 60});
    }
  }
