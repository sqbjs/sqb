...
class SerializerInstance {
  ...
  constructor(owner) {
    if (owner instanceof SerializerInstance) {
      ...
      this.nastedPool = owner.nastedPool;
      ...
      for (const plugin of plugins.items) {
        if (typeof plugin.createSerializer === 'function') {
          this.nastedPool = plugin.createSerializer(this.config);
          if (this.nastedPool) break;
        }
      }
    }
  }
  ...
  isReserved(s) {
    ...
    if (this.reservedWords.includes(s))
      return true;
    if (this.nastedPool && typeof this.nastedPool.isReserved === 'function')
      return this.nastedPool.isReserved(s);
  }
...
function __serialize(fnName, instance, obj, inf) {
  ...
  const plugin = instance.nastedPool;
