...
class Serializer {
  ...
  constructor(config) {
    ...
    for (const plugin of plugins.items) {
      if (typeof plugin.createSerializer === 'function') {
        this.nastedSerializer = plugin.createSerializer(self._config);
        if (this.nastedSerializer) break;
      }
    }
  }
  ...
  generate(query, inputValues) {
    ...
    instance.generate(query, inputValues);
    return {
      sql: instance.sql,
...
class SerializerInstance {
  ...
  constructor(owner) {
    ...
    this.nastedSerializer = owner.nastedSerializer;
  }
  ...
  isReserved(s) {
    ...
    if (this.reservedWords.includes(s))
      return true;
    if (this.nastedSerializer && typeof this.nastedSerializer.isReserved === 'function')
      return this.nastedSerializer.isReserved(s);
  }
  ...
  generate(query, inputValues) {
    ...
    this.sql = flattenText(sql, {noWrap: !this.config.prettyPrint});
    return this.sql;
  }
...
function __serialize(fnName, instance, obj, inf) {
  ...
  const plugin = instance.nastedSerializer;
