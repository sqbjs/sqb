...
class Serializer {
  ...
  build(query, values) {
    ...
    const sql = this._serializeSqlObject(query);
    return {
   ...
   * @protected
   */
  _serializeSelect(query) {
    const self = this;
    let out = 'select';
    let sep;

    // columns part
    let s = self._serializeColumns(query._columns, {section: 'select.columns'}) ||
        '*';
    ...
    // from part
    s = self._serializeFrom(query._tables, {section: 'select.from'});
    if (s) {
      if (s.includes('\n'))
        sep = '\n';
      out += sep + s;
    ...
    s = self._serializeJoins(query._joins, {section: 'select.joins'});
    if (s) {
      out += '\n' + s;
      sep = '\n';
    }

    s = self._serializeWhere(query._where, {section: 'select.where'});
    if (s) {
      if (sep === '\n' || s.length > 60) {
        out += '\n' + s;
      } else {
    ...
    s = self._serializeGroupBy(query._groupby, {section: 'select.groupby'});
    if (s)
      out += '\ngroup by ' + s;

    s = self._serializeOrderBy(query._orderby, {section: 'select.orderby'});
    if (s)
      out += '\n' + s;
    return out;
  }
   ...
   * @protected
   */
  _serializeInsert(query) {
    assert.ok(query && query._table && ['raw', 'table'].includes(query._table.type),
        'Invalid argument. Only Raw or TableName allowed in "insert(?)"');
    assert.ok(!!query._values, 'values required for Insert query');

    const self = this;
    let out = 'insert into ' +
        self._serializeSqlObject(query._table, {section: 'insert.table'}) +
        ' (' +
        self._serializeColumns(query._columns, {section: 'insert.columns'}) +
        ')';
  ...
  _serializeUpdate(query) {
    ...
    const self = this;
    let out = 'update ' +
        self._serializeSqlObject(query._table, {section: 'update.table'}) +
        ' set\n\t';

    // Serialize update values
    if (query._values.isRaw) {
      out += self._serializeRaw(query._values, {section: 'update.values'});
    } else {
    ...
    const s = this._serializeWhere(query._where, {section: 'update.where'});
    if (s)
      out += '\n' + s;
   ...
   * @protected
   */
  _serializeDelete(query) {
    assert.ok(query && query._table && ['raw', 'table'].includes(query._table.type),
        'Invalid argument. Only Raw or TableName allowed in "delete(?)"');
    const self = this;
    let out = 'delete from ' +
        self._serializeSqlObject(query._table, {section: 'delete.table'});

    // Serialize conditions
    const s = self._serializeWhere(query._where, {section: 'delete.where'});
    if (s)
      out += (s.length > 60 ? '\n' : ' ') + s;
