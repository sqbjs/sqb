...
import { MariadbAdapter } from '@sqb/mariadb';
...
export class MariadbMigrationAdapter extends MigrationAdapter {
  ...
  static async create(
  ...
  ): Promise<MariadbMigrationAdapter> {
    // Same rationale as @sqb/migrator's MySQL adapter: a migration script
    // may contain several statements (e.g. a CREATE TABLE followed by a
    // CREATE TRIGGER whose body has its own internal ";"s), and the
    // `mariadb` driver only runs more than one statement per call when
    // multipleStatements is enabled at connection time. The server's own
    // parser correctly treats a trigger/procedure body as one statement, so
    // no script-splitting is needed here, unlike Oracle or MSSQL.
    const connection = (
    ...
    ).intlcon as MariadbDriverConnection;
    try {
      const adapter = new MariadbMigrationAdapter();
      ...
      // MariaDB's CREATE SCHEMA is just CREATE DATABASE, same as MySQL -
      // no DBA-level provisioning needed, unlike Oracle.
...
(
    ...
    CONSTRAINT pk_${adapter.summaryTable} PRIMARY KEY (package_name)
      ...
      // Auto-increment is a native column attribute in MariaDB - no
...
(
  ...
  async writeEvent(event: MigrationAdapter.Event): Promise<void> {
    ...
    await this._connection.query(sql, {
      ...
      event: event.event,
      // Unlike mysql2, the mariadb driver rejects a named placeholder whose
      // bound value is `undefined` outright ("Placeholder 'x' is not
      // defined") instead of treating it as NULL - confirmed live.
   ...
   * Session-scoped, so it isn't released by a migration's DDL - MariaDB,
   * like MySQL, implicitly commits on CREATE/ALTER/DROP TABLE, which would
   * otherwise release a transaction-held lock the instant a migration's
   * first DDL statement ran.
   *
   * Unlike MySQL, MariaDB's GET_LOCK() rejects a negative timeout outright
   * (it warns "Incorrect timeout value" and returns NULL instead of waiting
   * forever) - confirmed against a live server. A very large positive
   * timeout (2^30 seconds, ~34 years) is used instead to get the same
   * effectively-infinite wait.
   */
  async lockSchema(): Promise<void> {
    await this._connection.query('SELECT GET_LOCK(:name, :timeout)', {
      name: this._lockName(),
      timeout: 1073741824,
    });
