import {
    ...
    isLogicalOperator,
    isCompOperator,
    Select, Raw, Exists, In, LeftOuterJoin, Count, Delete
} from '@sqb/builder';
...
export class Repository<T> {
    ...
    async remove(keyValue: T | any | Record<string, any>): Promise<boolean> {
        return !!(await this._remove({
            entityDef: this._entityDef,
            filter: [this._getPrimaryKeyConditions(keyValue)]
        }));
    }
    ...
    protected async _create(instance: Partial<PickWritable<T>>,
                            getAutoGeneratedColumns?: boolean): Promise<QueryResult> {
        const input = {};
        const values = {};
        const returning: string[] | undefined = getAutoGeneratedColumns ? [] : undefined;
        let v;
        for (const col of this._entityDef.columns.values()) {
            v = instance[col.name];
            if (isDataColumn(col)) {
                if (returning && v === undefined && col.autoGenerate)
                    returning.push(col.fieldName);
                if (v === undefined || !(col.insert == null || col.insert))
                    continue;
    ...
    getKeyValues(instance: T) {
        ...
        const instanceKeysUpper = instanceKeys.map(x => x.toUpperCase());

        if (Array.isArray(primaryIndex.column) && primaryIndex.column.length > 1) {
            const indexColumns = primaryIndex.column;
            const out: Record<string, any> = {};
            for (let k of indexColumns) {
                k = k.toUpperCase();
                const i = instanceKeysUpper.indexOf(k);
                if (i < 0)
                ...
                out[k] = instance[instanceKeys[i]];
            }
            return out;
        }
        const primaryColumnName = primaryIndex.column as string;
        const col = entityDef.getColumn(primaryColumnName);
        if (!isDataColumn(col))
            throw new Error(`Primary column for "${entityDef.name}" entity not found`);

        const i = instanceKeysUpper.indexOf(primaryColumnName.toUpperCase());
        if (i < 0)
            throw new Error(`Value of key field "${this._entityDef.name}.${primaryColumnName}" required to perform this operation`);
        return {[col.name]: instance[instanceKeys[i]]};
    }

    protected _getPrimaryKeyConditions(keyValue: any | Record<string, any>): Operator {
        const entityDef = this._entityDef;
        const primaryIndex = entityDef.primaryIndex;
        if (!primaryIndex)
            throw new Error(`no primary fields defined for "${entityDef.name}" entity`);
        ...
        if (Array.isArray(primaryIndex.column) && primaryIndex.column.length > 1) {
            ...
            for (let k of keyColumns) {
                k = k.toUpperCase();
                const i = valueKeysUpper.indexOf(k);
                if (i < 0)
                    throw new Error(`Value of key field "${this._entityDef.name}.${k}" required to perform this operation`);
                // const col = entityDef.getColumn(k);
                // eslint-disable-next-line
        ...
        if (typeof keyValue === 'object') {
            ...
            if (i < 0)
                throw new Error(`Value of key field "${this._entityDef.name}.${primaryColumnName}" required to perform this operation`);
            return Eq(col.fieldName, keyValue[valueKeys[i]]);
        }
