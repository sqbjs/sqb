...
export function extractKeyValues<T>(
  ...
  if (!primaryIndex) throw new Error(`No primary fields defined for "${entityDef.name}" entity`);
  ...
  if (primaryIndex.columns.length > 1) {
    ...
    for (const k of primaryIndex.columns) {
      ...
      if (i < 0) throw new Error(`Value of key field "${entityDef.name}.${k}" required to perform this operation`);
      validateCol(k);
      out[k] = valueOrInstance[valueKeys[i]];
    }
    if (keepOther) {
      for (let i = 0; i < valueKeys.length; i++) {
        if (primaryIndex.columns.find(x => x.toUpperCase() === valueKeysUpper[i])) continue;
