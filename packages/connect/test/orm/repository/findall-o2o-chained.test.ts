...
import {Association, Entity, hasOne} from '@sqb/connect';
...
class Customer2 extends Customer {
    @Association(hasOne(Country).hasOne(Continent))
    continent: Continent;
}

describe('findAll() one-to-one link-chained associations', function () {
    ...
    it('should return instance from multi level associations', async function () {
    ...
    it('should filter by multi level associated column', async function () {
        const repo = client().getRepository(Customer2);
        const rows = await repo.findAll({
            elements: ['id', 'countryCode'],
    ...
    it('should order by multi level associated column', async function () {
        const repo = client().getRepository(Customer2);
        const rows = await repo.findAll({
